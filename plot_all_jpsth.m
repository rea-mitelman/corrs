...
for isess=1:length(table)
    ...
    unit_cat_count.CTX=zeros(1,length(all_mats.CTX));
    unit_cat_count.Thal=zeros(1,length(all_mats.Thal));
    ...
    total_cat_count.CTX=total_cat_count.CTX + sum(repmat(unit_cat_count.CTX,3,1)==repmat([1:3]',1,length(unit_cat_count.CTX)),2);
    total_cat_count.Thal=total_cat_count.Thal + sum(repmat(unit_cat_count.Thal,3,1)==repmat([1:3]',1,length(unit_cat_count.Thal)),2);
end
