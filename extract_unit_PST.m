function [tmpspikes,PstPro, Npro, frest_pro, PstSup, Nsup, frest_sup] = extract_unit_PST( table, isess, iunit, PDmin, PDmax, Tmin, Tmax, ev2take)
...
for j=table(isess).extens(1):table(isess).extens(2) %runs on all files in session
	...
	for ti=1:size(fdat.trials,1)
		...
		if flag && ~isempty(evZero),
			...
			tmpSpikes=Tspike(find(Tspike>=refTime+Tmin & Tspike<=refTime+Tmax))-refTime-Tmin+1;
			spikes(trialCounter,:)=[turn2row(time2bin(round(tmpSpikes),Tmax-Tmin+1))];
		else
			spikes(trialCounter,:)=zeros(1,duration);
		end
		handPosition(trialCounter)=fdat.hand_position;
		targets(trialCounter)=t;
		% now computing the length of the hold period
		[etmp, itmp] = returnEvPosition(events, t, 'ho');
		etmp = etmp+fdat.trials(ti,1)-1;
		ttmp1 = fdat.events_time(etmp);
		[etmp, itmp] = returnEvPosition(events, t, 'hof');
		etmp = etmp+fdat.trials(ti,1)-1;
		ttmp2 = fdat.events_time(etmp);
		if ~isempty(ttmp1) & ~isempty(ttmp2),
			Thold(trialCounter) = ttmp2-ttmp1;
			% %             if Thold(trialCounter) > 4000,
			% %                 disp('here');
			% %             end
		else
			Thold(trialCounter) = 0;
		end
		...
		if flag & ~isempty(evZero),
			refTime=fdat.events_time(evZero);
			tmpSpikes=Tspike(find(Tspike>=refTime+restTmin & Tspike<=refTime+restTmax))-refTime-restTmin+1;
...
if ~isnan(PD(2)) & PDsig(2) < 0.05,
	...
	PstSup = sum(tmpspikes(find(tmptarget == pdindx),:));
