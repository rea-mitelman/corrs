function [spk_mat_all, PD, PDsig, RespDirect, RespSig] = ...
    extract_PST_mat_PD_resp( table, isess, iunit, PDmin, PDmax, Tmin, Tmax, ev2take, restTmin, restTmax)
% Returns the following outputs
% spk_mat_all - a matrix of all valid trials
% PD - prefered direction
% PDsig - P value of the prefered direction
% RespDirect - Responisiveness direction (FR rest >/< FR after event)
% RespSig - P value of the responsiveness


% Rest time before reference time (?)
if ~exist('restTmin','var') || isempty(restTmin)
    restTmin = -500;
end

if ~exist('restTmax ','var') || isempty(restTmax)
    restTmax = 0;
end
...
reast_fr_vec=sum(restspikes(PronIndex,:),2)'/restduration*1000; %vector of rest fr per valid trials
...
ev_fr_vec=rp/(PDmax-PDmin+1)*1000;

RespDirect = sign(mean(rp-reast_fr_vec)); %RespDirect>0 ==> average increase response and vice versa
[~,RespSig]=ttest(rp,reast_fr_vec);
...
%% Suppination trials - irrelevant for the current experiment
...
switch (lower( monks)),
    ...
    case 'y'
        filepath = 'G:\users\ream\Prut\Yolanda\Data\YolandaData\y150212\MergedEdFiles\';
        disp('===============================================================')
        warning('This directory is only relevant for subgroup of the data (for SFN poster)!!!')
        disp('===============================================================')
        
    otherwise
        
        error('Unknown monkey letter %s',monks)
end
